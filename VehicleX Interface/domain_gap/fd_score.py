...
def _compute_statistics_of_path(opt, path, model, batch_size, dims, cuda):
    if path.endswith('.npz'):
        ...
        files = files[:2000]
        m, s = calculate_activation_statistics(opt, files, model, batch_size,
                                               dims, cuda) 
    return m, s
...
def calculate_fd_given_paths(paths, opt):
    ...
    if opt.FD_model == 'posenet':
        dims = 2048
        model = get_pose_net(num_layers = 50)
        pretrained_dict = torch.load("./domain_gap/models/pose_resnet_fea.pth")
    ...
    m1, s1 = _compute_statistics_of_path(opt, paths[0], model, 16,
                                         dims, cuda)
    ...
    m2, s2 = _compute_statistics_of_path(opt, paths[1], model, 16,
                                         dims, cuda)
...
if __name__ == '__main__':
    ...
    fid_value = calculate_fd_given_paths(args.path,
                                          args.batch_size,
                                          args.gpu != '',
                                          8192)
    print (fid_value)
