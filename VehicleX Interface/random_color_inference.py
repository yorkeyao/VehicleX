...
parser.add_argument('--env_path', type=str, default='./Build-win-color/VehicleX.exe')
parser.add_argument('--out_lab_file', type=str, default='VeRi_label.xml')
parser.add_argument('--id_number', type=int, default=1362)
parser.add_argument('--model_number', type=int, default=178)
...
vehicle_info = get_color_distribution(opt.id_number, opt.model_number)
...
def get_save_images_by_attributes(attribute_list, control_list, cam_id, dataset_size, output_dir):
    ...
    while cnt < dataset_size:
        ...
        color_R = vehicle_info['R'][cnt % opt.id_number]
        color_G = vehicle_info['G'][cnt % opt.id_number]
        color_B = vehicle_info['B'][cnt % opt.id_number]
        env_info = env.step([[vehicle_info['model'][cnt % opt.id_number], angle[z], temp_intensity_list[z], temp_light_direction_x_list[z], Cam_distance_y_list[z], Cam_distance_x, Cam_height_list[z], scene_id, train_mode, color_R, color_G, color_B]])[default_brain] 
        done = env_info.local_done[0]
        car_id = cnt % opt.id_number
